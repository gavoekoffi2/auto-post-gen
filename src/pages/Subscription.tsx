...
import { PAYMENT_ACCOUNTS, PAYMENT_BENEFICIARY, SUPPORT_EMAIL } from "@/lib/appConfig";
import {
  ENTITLEMENT_COLUMNS,
  PAYMENT_METHODS,
  PLAN_LIMITS,
...
type SubscriptionRequest = Tables<"subscription_requests">;
...
export default function Subscription() {
  usePageMeta("Abonnement", "Votre forfait Pro Social AI, son échéance et son renouvellement.");
  ...
  const [method, setMethod] = useState<PaymentMethod | "">(PAYMENT_ACCOUNTS[0]?.method ?? "");
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: profileRow }, { data: requestRows }] = await Promise.all([
      supabase.from("profiles").select(ENTITLEMENT_COLUMNS).eq("id", session.user.id).maybeSingle(),
      supabase
    ...
    setProfile(profileRow as SubscriptionFields | null);
    setRequests(requestRows || []);
    setLoading(false);
    return profileRow as SubscriptionFields | null;
  }, [navigate]);
  ...
  const account = PAYMENT_ACCOUNTS.find((a) => a.method === method) || null;
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const { data, error } = await supabase.functions.invoke("request-subscription", {
        body: { plan, billingPeriod: period, paymentMethod: method, payerPhone, paymentReference: reference },
      });
      if (error) throw new Error(await functionErrorMessage(error, "Impossible d'envoyer votre demande."));
      toast.success(
  ...
  const cancelPending = async () => {
    ...
    try {
      const { error } = await supabase.functions.invoke("request-subscription", {
        body: { action: "cancel", requestId: pending.id },
      });
      if (error) throw new Error(await functionErrorMessage(error, "Impossible d'annuler la demande."));
                ...
                <Card className="glass-card p-5 space-y-4">
                  ...
                  {account && (
                    <div className="rounded-xl bg-muted/40 p-4 space-y-2 text-sm">
                      <p>
                        Envoyez <strong>{fcfa(amount)}</strong> à <strong>{PAYMENT_BENEFICIARY}</strong> par{" "}

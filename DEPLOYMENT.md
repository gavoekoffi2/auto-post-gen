...
| `SMTP_HOST` / `SMTP_PORT` / `SMTP_USER` / `SMTP_PASS` / `SMTP_SENDER_EMAIL` | GitHub Actions secrets (not Supabase) | Transactional sender for Supabase Auth. **Setting all four of HOST/USER/PASS/SENDER_EMAIL turns email confirmation ON automatically** on the next deploy; until then `mailer_autoconfirm` stays enabled and anyone can sign up with an address they do not own. With Resend: `SMTP_HOST=smtp.resend.com`, `SMTP_PORT=465`, `SMTP_USER=resend`, `SMTP_PASS=<your RESEND_API_KEY>`, `SMTP_SENDER_EMAIL=no-reply@yourdomain` (domain must be verified in Resend). |

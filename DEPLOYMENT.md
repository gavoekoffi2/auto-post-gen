...
| `ZERNIO_ALLOW_SHARED_PROFILE` *(optional)* | `zernio-connect` | Leave unset. When the Zernio plan cannot create a dedicated profile for a new user, connection now fails instead of silently reusing the operator's default profile (users sharing a profile would publish to and read comments from each other's accounts). Set to `true` only for a single-tenant deployment. |

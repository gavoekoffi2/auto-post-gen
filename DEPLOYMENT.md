...
| `RESEARCH_NEWS_REGION` / `RESEARCH_NEWS_LANG` *(set these)* | `generate-content`, `auto-generate-weekly` | Which market the free news research targets. Defaults to `CI` / `fr`. This was previously hard-coded to France, so generated posts were grounded in French news regardless of where the customer trades — set it to the country you actually sell in. |
...
> so `AYRSHARE_API_KEY`, `POSTIZ_API_KEY`, `POSTIZ_API_URL`,
> `OAUTH_STATE_SECRET`, `OAUTH_LINKEDIN_*`, `OAUTH_META_*` and
> `OAUTH_TWITTER_*` are read by nothing. A credential no code uses cannot help
> you and can still leak — remove them from the project's secrets.

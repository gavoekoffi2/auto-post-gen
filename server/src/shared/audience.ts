...
export function buildAudiencePrompt(audiences: unknown, preferredIndex = 0): string {
  ...
  // normalized is non-empty here (checked above), but the index signature
  // cannot know that; falling back keeps the prompt valid either way.
  if (!audience) {
    return `\nCIBLE PRIORITAIRE: lecteurs réellement concernés par l'activité décrite.\n`;
  }
  return `
═══════════════════════════════════════════════════════════

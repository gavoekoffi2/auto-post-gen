...
test("concurrent status reads settle a render once", async () => {
  ...
  // Depending on PostgreSQL scheduling, the second reader either observes the
  // still-processing row (two provider reads) or the first reader's completed
  // row (one provider read). Both are correct; the invariant is one settlement
  // and no provider read after the terminal state is stored.
  const concurrentReadCount = statusReads;
  assert.ok(concurrentReadCount >= 1 && concurrentReadCount <= 2, "the provider was read at most once per concurrent caller");
  ...
  assert.equal(statusReads, concurrentReadCount);
});
